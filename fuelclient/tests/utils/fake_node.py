...
def get_fake_node(cluster=None, fqdn=None, node_id=None, cpu_model=None,
                  roles=None, mac=None):
    """Creates a fake random node

    Returns the serialized representation of a dumped Fuel node
    with customized parameters. Represents the average amount of
    data for a single node.

    """
    node_name = fqdn or tests.utils.random_string(10, prefix='fake-node-')

    return {"cluster": cluster or 1,
            "error_type": None,
